...
const chooseBestSum = (max, len , arr) => {
    let sum = getPermutations(arr).reduce((a,c) => getSum(c) > a && c.length == len && getSum(c) <= max ? getSum(c) : a, 0)
...
const getPermutations = (arr) => {
    let returnArray = []
    let x = function(pre, rest){
        for (let i = 0; i < rest.length; i++){
            returnArray.push(pre.concat(rest[i]))
            x(pre.concat(rest[i]), rest.slice(i + 1))
        }
    }
    x([], arr)
    return returnArray
...
console.log(chooseBestSum(3,3,[91, 74, 73, 85, 73, 81, 87]))