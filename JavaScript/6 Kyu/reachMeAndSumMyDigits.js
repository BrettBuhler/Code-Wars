...
const sumDigNthTerm = (interval, patternl, nthterm) => {
    let index = 0
    for (let i = 0; i < nthterm - 1; i++){
        if (index == patternl.length){
            index = 0
        }
        interval += patternl[index]
        index++
    }
    return interval.toString().split('').reduce((a,b)=>parseInt(a) + parseInt(b))
}

console.log(sumDigNthTerm(10, [2, 1, 3], 157))