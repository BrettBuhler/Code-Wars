 ...
 * @param {an array of integers greater or equal to zero} array 
...
const canJump = (array) => {
    ...
    if (array[0] >= array.length) return true
    for (let i = 0; i < array[0]; i++){
        if (array[i+1] >= 0){
            const nextStep = canJump(array.slice(i+1))
